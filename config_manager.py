    ...
    if os.path.exists(config_path):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to create default config after load failure: {save_error}", exc_info=True)
                raise ConfigError("Failed to load or create default configuration.") from save_error
            raise ConfigError(f"Failed to load configuration from {config_path}: {e}") from e #Raise the config error
    else:
        logger.warning(f"Configuration file not found: {config_path}. Creating default config.")
        try:
            save_config()
            logger.info(f"Default configuration created: {config_path}")
        except ConfigError as save_error:
            logger.error(f"Failed to create default config: {save_error}", exc_info=True)
...
def save_config():
    ...
    try:
        ...
        raise ConfigError(f"Failed to save configuration to {config_path}: {e}") from e #Raise the config error
