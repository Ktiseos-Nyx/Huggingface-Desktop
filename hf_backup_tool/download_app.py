...
class DownloadApp(QWidget):
    def __init__(self):
        ...
        try:
            ...
            logger.error(f"Error loading max_concurrent_downloads from config: {e}. Defaulting to 1.")
            self.max_concurrent_downloads = 1
            QMessageBox.warning(self, "Config Warning", f"Could not load max concurrent downloads setting. Defaulting to 1. Error: {e}")
    ...
    def update_button_states(self):
        ...
        if selected_items:
            ...
            try:
                ...
                logger.error(f"Exception in update_button_states (for remove button logic for item '{selected_text}'): {e}", exc_info=True)
    ...
    def _process_queue(self):
        while len(self.active_workers) < self.max_concurrent_downloads and self.download_queue:
            task_to_start = self.download_queue.popleft()
            task_to_start.status = "Downloading"
            self.task_map[task_to_start.id] = task_to_start
            
            self.output_text.append(f"Starting download: {task_to_start.repo_url} (ID: {task_to_start.id})")
    ...
    def on_download_finished(self, task_id, success, message):
        logger.info(f"Download task {task_id} finished. Success: {success}. Message: {message}")
    ...
    def handle_cancel_all_tasks(self):
        ...
        if active_tasks_cancelled_count > 0:
            self.output_text.append(f"Cancellation requested for {active_tasks_cancelled_count} active download(s).")
        ...
        if pending_tasks_cleared_count > 0:
            self.output_text.append(f"Cleared {pending_tasks_cleared_count} pending download(s).")
        ...
        try:
            ...
            if task:
                if task_id_to_remove in self.active_workers:
                    self.output_text.append(f"Cannot remove active task {task_id_to_remove}. Cancel it first.")
                    return
    ...
    def clear_download_queue(self):
        ...
        reply = QMessageBox.question(self, "Clear Downloads", 
